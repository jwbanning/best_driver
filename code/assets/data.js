$(document).ready(function() {
 ...
 //Handle Markers on Scale ---------------------------------
  ...
  function setMarkerSelected(i) {
    ...
    var text = ' <svg class="labelSvg"><g><rect></rect><text data-index="'+i+'" text-anchor="left" x="'+(parseInt(path.attr('cx'))+20)+'" y="'+path.attr('cy')+'" style="fill: #000; font-family: "Open Sans"; font-size: 13px;">'+model.viewModel.locations()[i].location().City +', '+model.viewModel.locations()[i].location().State +'  </text></g></svg>';
    model.viewModel.map.setSelectedMarkers(i);
    $('circle[data-index="'+i+'"]').css('fill', model.viewModel.color);
    $(pathParent).append(text);
  }
